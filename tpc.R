...
tpdag <- function(skel, order) {
  ...
  pcalg::addBgKnowledge(vOrientTemporal(tempSkelAmat, skel@sepset), checkInput = FALSE) # Algorithm step 3.1 into 3.2
}



orderRestrictAmat <- function(amat, order) { # Algorithm step 3.0
  p <- nrow(amat)
...
#' 
vOrientTemporal <- function(amat, sepsets) { # Algorithm step 3.1
  vnames <- rownames(amat)
  nvar <- nrow(amat)
    ...
    if (length(theseAdj) >= 2) {
      ...
      if (npairs >= 1) {
        ...
        for (j in 1:npairs) {
          ...
          j1 <- thisPair[1]
          j2 <- thisPair[2]
          thisPairAdj <- j2 %in% findAdjacencies(amat, j1) # Are the two neighbours adjacent to each other?
          ...
          if (!thisPairAdj) {
            ...
            if (!(i %in% sepset1) & !(i %in% sepset2)) {
              ...
              if (amat[i,j1] == 1 & amat[i,j2] == 1) { # If we have not already ruled out that j1 -> i may be true and j2 -> may be true, then that is the case.
                amat[j1, i] <- 0
                amat[j2, i] <- 0
              }
            }
